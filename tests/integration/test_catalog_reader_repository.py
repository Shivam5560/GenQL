...
def test_reads_tables_and_views(shop_schema: Engine) -> None:
    ...
    objects = {o.object_name: o for o in repo.read_objects(ref)}
...
def test_reads_columns_with_primary_key_flag(shop_schema: Engine) -> None:
    repo = PostgresCatalogReaderRepository(shop_schema)
    ref = SchemaRef(datasource_name="local", schema_name="shop")
    columns = {c.qualified_name: c for c in repo.read_columns(ref)}
    pk = columns["local.shop.customer.c_customer_sk"]
    assert pk.is_primary_key is True
    assert pk.is_nullable is False
    assert pk.ordinal == 1
    assert columns["local.shop.orders.o_amount"].data_type == "numeric(10,2)"


def test_reads_foreign_key_with_referenced_object(shop_schema: Engine) -> None:
    repo = PostgresCatalogReaderRepository(shop_schema)
    ref = SchemaRef(datasource_name="local", schema_name="shop")
    fks = [c for c in repo.read_constraints(ref) if c.constraint_type is ConstraintType.FOREIGN_KEY]
    assert len(fks) == 1
...
def test_primary_key_constraint_has_column_names_but_no_referenced_columns(
    ...
    repo = PostgresCatalogReaderRepository(shop_schema)
    ref = SchemaRef(datasource_name="local", schema_name="shop")
    pks = [c for c in repo.read_constraints(ref) if c.constraint_type is ConstraintType.PRIMARY_KEY]
...
def test_reltuples_sentinel_is_mapped_to_none_not_zero(engine: Engine) -> None:
    ...
    ref = SchemaRef(datasource_name="local", schema_name="unanalyzed")
    objects = {o.object_name: o for o in repo.read_objects(ref)}
    assert objects["fresh"].row_estimate is None

    with engine.begin() as conn:
        conn.execute(text("ANALYZE unanalyzed.fresh"))
    analyzed = {o.object_name: o for o in repo.read_objects(ref)}
